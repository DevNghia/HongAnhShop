...
class Product extends Controller
{
    ...
    public function show_detail(Request $request, $product_id)
    {
        ...
        $related_product =
            ...
            ->join('tbl_category_product', 'tbl_category_product.category_id', '=', 'tbl_product.category_id')
            ->join('tbl_brand_product', 'tbl_brand_product.brand_id', '=', 'tbl_product.brand_id')->where('tbl_brand_product.brand_id', $brand_id)->where('product_status', '1')->whereNotIn('tbl_product.product_id', [$product_id])->limit(3)->get();
        // foreach ($related_product as $key => $value) {

        // }
        return view('pages.product.show_details')->with('cate_product', $cate_product)->with('brand_product', $brand_product)->with('product', $product)->with('related_product', $related_product)->with(compact('meta_desc', 'meta_keywords', 'meta_title', 'url_canonical'));
    }
}
