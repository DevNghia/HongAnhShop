...
@section('content')
                    ...
                    <div class="shopper-info">
                        ...
                        @if (Session()->get('voucher'))
                            @foreach (Session()->get('voucher') as $key => $vou)
                                <input type="hidden" name="order_voucher" class="order_voucher"
                                    value="{{ $vou['voucher_code'] }}">
                            @endforeach
                        @else
                        ...
                        <form role="form" id="formValidation" action="insert-delivery" method="post">
                            ...
                            <input type="button" value="Tính phí vận chuyển" name="caculate_order" id=""
                            ...
                            @foreach ($content as $key => $v_content)
                                ...
                                <tr>
                                    <td class="cart_product">
                                        <a href=""><img src="{{ '/uploads/product/' . $v_content->options->image }}"
                                                width="50px" alt=""></a>
                                    </td>
                                    ...
                                    <td class="cart_quantity">
                                        <form action="/update-quantity-cart" method="post">
                                            ...
                                            <input class="cart_quantity_input" type="hidden" name="cart_quantity"
        ...
        <td>
            ...
            @if (Session()->get('voucher'))
                ...
                @foreach (Session()->get('voucher') as $key => $vou)
                    ...
                    @endif
                    Tiền sau giảm
                    <span>{{ number_format($total + Session()->get('fee') - $total_voucher) . ' ' . 'vnđ' }}</span>
                    <input type="hidden" name="order_total_after" class="order_total_after"
                        value="{{ $total + Session()->get('fee') - $total_voucher }}">
                @endforeach
            @else
                <input type="hidden" name="order_total_after" class="order_total_after"
                    value="{{ $total + Session()->get('fee') }}">
    ...
    <form action="{{ url('/vnpay_payment') }}" method="POST">
        @csrf
        <button type="submit" class="btn btn-primary btn-sm " name="redirect">Thanh toán bằng VNPAY</button>
    </form>
    <div class="container">
        {{-- <div class="heading">
